...
export default function MetricsCharts({
  ...
  const barData = useMemo(() => {
    return blocks.map((b) => {
      const avg = b.items.length === 0 ? 0 : Math.round(b.items.reduce((s, k) => s + k.difficulty, 0) / b.items.length);
...
<figure
  ...
  aria-labelledby="cap-intent"
>
  <figcaption id="cap-intent" className="sr-only">
    Pie chart of keywords by intent
  </figcaption>
  ...
  <div className="h-[260px]">
    {/* NOTE: no role/aria-label on ResponsiveContainer */}
...
<figure
  ...
  aria-labelledby="cap-source"
>
  <figcaption id="cap-source" className="sr-only">
    Bar chart of average keyword difficulty by source
  </figcaption>
